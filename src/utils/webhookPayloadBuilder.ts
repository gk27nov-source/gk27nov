 ...
 */
export function sanitizePhoneNumber(phoneStr?: string): { international: string; formatted: string } {
  if (!phoneStr || phoneStr.trim() === '') {
    return { international: '+919820194821', formatted: '+91 98201 94821' };
  }
 ...
 */
export function sanitizeEmail(emailStr?: string, defaultEmail = 'contact@apexsolutions.in'): string {
...
export function generateMessagesForEvent(
  ...
  const phone = data.contactNumber || data.phone || data.mobile || '+91 98201 94821';
...
export function buildWebhookPayload(
  ...
  const triggeredByEmail = options.triggeredByEmail || 'automation@apexsolutions.in';
  ...
  const { international: cleanPhone, formatted: formattedPhone } = sanitizePhoneNumber(rawPhone);
  ...
  const rawEmail =
    ...
    'rajesh.mehra@mehra-logistics.com';
