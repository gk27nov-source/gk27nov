...
export const AutomationCenterView: React.FC = () => {
  ...
  const [n8nWebhookUrl, setN8nWebhookUrl] = useState(
    settings.n8nWebhookUrl || 'https://n8n.yourdomain.com/webhook/business-hub'
  );
  const [n8nApiKey, setN8nApiKey] = useState(settings.n8nApiKey || 'n8n_sec_89df201934ba');
  ...
  const handleTestTrigger = async (rule: AutomationRule) => {
    ...
    try {
      await triggerAutomationRule(rule.id, {
        sampleCustomer: 'Rajesh Mehra (Mehra Logistics)',
        sampleTicket: 'TICK-2026-101',
        sampleLeadValue: 350000,
        triggeredBy: currentUser?.displayName || 'Admin',
      });
    } catch (e) {
      console.error(e);
    } finally {
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
        ...
        <button
          ...
          className="px-4 py-2 text-xs font-bold bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl border border-slate-200 transition-colors flex items-center gap-1.5"
        >
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="p-6 space-y-4 text-xs">
              <div>
                ...
                <pre className="p-3 bg-slate-900 text-emerald-400 rounded-xl font-mono text-[11px] overflow-x-auto max-h-52">
