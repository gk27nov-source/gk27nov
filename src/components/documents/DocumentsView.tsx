...
export const DocumentsView: React.FC = () => {
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    ...
    await addDocument({
      ...
      fileSize: uploadFile ? `${(uploadFile.size / (1024 * 1024)).toFixed(2)} MB` : '1.2 MB',
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      {downloadToast && (
        <div className="p-3 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl text-xs flex items-center justify-between">
          ...
          <button onClick={() => setDownloadToast(null)} className="text-emerald-600 hover:text-emerald-900 font-bold">✕</button>
        </div>
      )}
            ...
            <div className="space-y-3 pt-3 border-t border-slate-100 text-xs">
              ...
              <div className="flex items-center justify-between pt-1">
                ...
                <div className="flex items-center gap-1">
                  <button
                    onClick={() => {
                      setDownloadToast(`Preparing download for "${doc.title || doc.name}" (${doc.fileSize})`);
