    ...
    <>
      <header className="sticky top-0 z-30 flex items-center justify-between h-16 px-4 md:px-8 bg-white border-b border-slate-200">
        ...
        <div className="flex items-center gap-2 sm:gap-3.5">
          ...
          {/* Role Switcher Pill — demo builds only.
              This control changes the caller's role with no credential, so it
              is a full authentication bypass. Vite removes the branch from the
              bundle when VITE_DEMO_MODE is not "true"; switchRole() also
              refuses at the point of use (see AppContext). */}
          {DEMO_MODE && (
