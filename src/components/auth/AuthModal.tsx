...
import { describeAuthError, type AuthErrorText } from '../../utils/authErrors';
...
interface AuthModalProps {
  ...
  onClose: () => void;
  /**
   * Rendered as the application's sign-in gate rather than a dismissible
   * modal. Hides the close affordance — there is nothing behind it to return
   * to until the user is authenticated.
   */
  isGate?: boolean;
}

export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, isGate = false }) => {
  // setCurrentUser is intentionally not taken from context any more — identity
  // is set by the auth observer from token claims, never from this component.
  ...
  const [errorMsg, setErrorMsg] = useState<AuthErrorText | null>(null);
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        // Deliberately NOT calling setCurrentUser here.
        //
        // This used to assign role: 'business_admin' and
        // organizationId: 'org_smart_hub_01' in client code, for every account
        // that signed in. Identity is now resolved by the onAuthStateChanged
        // observer in AppContext, from the signed custom claims on the ID
        // token — the browser does not get to name its own role.
        setSuccessMsg('Signed in. Loading your workspace...');
        if (!isGate) setTimeout(onClose, 800);
      } else if (mode === 'signup') {
        const userCredential = await createUserWithEmailAndPassword(auth, email, password);
        await updateProfile(userCredential.user, { displayName });

        // The account exists, but it belongs to no organisation yet. Role and
        // organizationId are custom claims, which only an administrator can
        // set (scripts/set-claims.mjs). Self-assigning them here is exactly
        // the privilege escalation this change removes.
        setSuccessMsg(
          'Account created. An administrator needs to assign your organisation and role before you can sign in.'
        );
      } else if (mode === 'forgot') {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Identity comes from the token claims, not from here. See above.
      setSuccessMsg('Signed in with Google. Loading your workspace...');
      if (!isGate) setTimeout(onClose, 800);
    } catch (err) {
      setErrorMsg(describeAuthError(err));
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="p-6 space-y-4">
          {errorMsg && (
            <div className="p-3 text-xs bg-red-50 text-red-700 border border-red-200 rounded-lg flex items-start gap-2">
              ...
              <div className="space-y-1">
                ...
                {/*
                  Shown only for codes that are a project misconfiguration
                  rather than a user mistake — during a pilot the person at the
                  keyboard is usually the person who can go and fix it.
                */}
                {errorMsg.adminHint && (
                  <p className="text-[11px] text-red-600/80 leading-relaxed">{errorMsg.adminHint}</p>
                )}
              </div>
            </div>
          )}
          ...
          {/* Quick Demo Personas — demo builds only. Vite strips this branch
              from the bundle entirely when VITE_DEMO_MODE is not "true". */}
          {DEMO_MODE && (
