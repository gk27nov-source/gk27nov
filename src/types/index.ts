...
export interface AutomationRule {
  ...
  triggerEvent: 
    ...
    | 'followup_due'
    | 'callback_requested'
    // Time-based events, added with the scheduler.
    //
    // invoice_due and payment_reminder already existed as payload names inside
    // the message generator, but were not bindable rule triggers — so no rule
    // could ever fire them. sla_at_risk is new: the existing "Complaint
    // Escalation & SLA Alert" rule is bound to new_complaint, while its own
    // description describes crossing an 80% SLA threshold, which is a
    // condition only a scheduled sweep can detect.
    | 'invoice_due'
    | 'payment_reminder'
    | 'sla_at_risk';
  isEnabled: boolean;
